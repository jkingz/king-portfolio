...
<template>
  <svg
    ...
    <g class="nuxites-grid-group">
      <template v-for="x in grid[0]" :key="x">
        <path
          v-for="i in grid[1]" :key="i"
          fill="currentColor"
          opacity=".2"
          class="glide-grid-item"
          :d="`M${(i - 1) * 32 + 5} ${(x - 1) * 32 + 10}l1.806,-2.951l-5,2.951l3.936,1.049l-0.742,-1.049z`"
        />
      </template>
